...
if ($method === 'GET') {
    try {
        ...
        $headerTenant = $_SERVER['HTTP_X_TENANT_ID'] ?? $_GET['restaurant_id'] ?? $_GET['tenant_id'] ?? null;
        ...
        } elseif ($headerTenant !== null && is_numeric($headerTenant) && (int)$headerTenant > 0) {
            $whereConditions[] = "f.restaurant_id = ?";
            $params[] = (int)$headerTenant;
        }
