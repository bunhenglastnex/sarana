...
export const useDeliveryStore = create<DeliveryState>((set, get) => ({
  ...
  fetchLiveOrders: async () => {
    try {
      ...
      const res = await Api.get(`/delivery.php${staffIdQuery}`);
      if (res.success && res.data) {
        ...
        if (Array.isArray(res.data.orders)) {
          ...
          const itemsSummary = Array.isArray(o.items)
            ? o.items.map((i: any) => `${i.quantity}x ${i.food_name}`).join(", ")
          ...
          return {
            ...
            distance: "1.8 mi",
            eta: "10 min",
            address: o.delivery_address || "Phnom Penh Central",
        ...
        if (liveOrders.length > 0) {
          ...
          set({
            availableOrders:
              available.length > 0 ? available : get().availableOrders,
            myDeliveries:
              assigned.length > 0 ? assigned : get().myDeliveries,
          });
        }
      }
    }
