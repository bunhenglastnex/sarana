...
export const CategoryGrid: React.FC<CategoryGridProps> = ({
  ...
  const getCategoryImageUrl = (cat: CategoryRecord) => {
    const raw = cat.image_url || cat.imageUrl;
    if (!raw) return null;
    if (raw.startsWith("/")) {
      ...
      {viewMode === "grid" ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-space-md items-stretch">
          {filteredCategories.map((cat) => {
            const imgSrc = getCategoryImageUrl(cat);
            return (
              <div
                key={cat.id}
                className={`bg-surface-container-lowest rounded-2xl shadow-xs border transition-all flex flex-col justify-between p-space-md relative overflow-hidden ${
                  cat.isActive
              ...
              >
                <div>
                  {/* Top Row: Category Avatar (Image or Icon) + Title + Active Toggle */}
                  <div className="flex items-center justify-between gap-2 mb-3">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="w-11 h-11 rounded-xl bg-surface-container-low border border-border/30 flex items-center justify-center text-xl shadow-xs shrink-0 overflow-hidden">
                        {imgSrc ? (
                          <img
                            src={imgSrc}
                            alt={cat.name}
            ...
            <table className="w-full text-left font-body-sm text-xs">
              <thead>
                <tr className="bg-surface-container-low text-on-surface-variant font-label-sm text-[11px] uppercase tracking-wider border-b border-border/30">
                  <th className="py-3 px-space-md">Category &amp; Image</th>
                    ...
                    >
                      <td className="py-3.5 px-space-md">
                        <div className="flex items-center gap-2.5">
                          <div className="w-9 h-9 rounded-lg bg-surface-container-low border border-border/30 flex items-center justify-center text-lg shrink-0 overflow-hidden">
                            {imgSrc ? (
                              <img
                                src={imgSrc}
                                alt={cat.name}
                          ...
                          <div>
                            ...
                            {cat.description && (
                              <div className="font-body-sm text-[10px] text-on-surface-variant line-clamp-1">
                                {cat.description}
                              </div>
                            )}
                          </div>
