...
export const FloatingCartBar: React.FC<FloatingCartBarProps> = ({
  ...
  const realItemCount = useMemo(() => {
    if (propItemCount !== undefined) return propItemCount;
    return items.reduce((count, item) => count + item.quantity, 0);
  }, [items, propItemCount]);

  const realTotalPrice = useMemo(() => {
    if (propTotalPrice !== undefined) return propTotalPrice;
    return items.reduce(
      (sum, item) =>
        sum +
        ...
        <div className="flex items-center gap-space-sm">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs font-bold text-inverse-on-surface">
              {realItemCount} {realItemCount === 1 ? 'item' : 'items'} • ${realTotalPrice.toFixed(2)}
            </span>
          </div>
        </div>
